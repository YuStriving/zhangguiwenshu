...
async def filter_table(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    write({"type":"progress","step":"表信息过滤","status":"running"})
    ...
    try:
        ...
        for table_item in table_infos:
            if table_item['name'] in result:
                # 保留所有主外键字段（role 为 primary 或 foreign）
                key_columns = [column_info for column_info in table_item['columns'] 
                              if column_info.get('role') in ['primary', 'foreign']]
                
                # 保留 LLM 选择的相关字段
                selected_columns = [column_info for column_info in table_item['columns'] 
                                   if column_info['name'] in result[table_item['name']]]
                ...
                table_item['columns'] = merged_columns
                filtered_table_infos.append(table_item)
        logger.info(f"过滤后的表信息数量：{len(filtered_table_infos)}")
        
        write({"type":"progress","step":"表信息过滤","status":"success"})
        return {"table_infos": filtered_table_infos}
    except Exception as e:
        logger.error(f"表信息过滤过程中出错: {e}")
        write({"type":"progress","step":"表信息过滤","status":"error"})
