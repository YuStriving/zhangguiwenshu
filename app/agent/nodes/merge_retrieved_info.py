...
async def merge_retrieved_info(state: DataAgentState, runtime: Runtime[DataAgentContext]) -> DataAgentState:
    ...
    write({"type":"progress","step":"合并信息","status":"running"})
    ...
    try:
        ...
        write({"type":"progress","step":"合并信息","status":"success"})
        logger.info(f"信息合并完成，共处理{len(table_info_list)}个表，{len(metric_info_list)}个指标")
        return {
            'table_infos':table_info_list,
            'metric_infos':metric_info_list
        }
    except Exception as e:
        logger.error(f"信息合并过程中出错: {e}")
        write({"type":"progress","step":"合并信息","status":"error"})
